...
\begin{minipage}{6in}
\centering
Wei-Chen Chen$^1$, George Ostrouchov$^1$, Drew Schmidt$^2$, \\
Pragneshkumar Patel$^2$, Hao Yu$^3$
\end{minipage}
\\
\\
$^1$Scientific Data Group, Computer Science and Mathematics Division, \\
...
\input{./pbdMPI-include/02-introduction}
\input{./pbdMPI-include/03-performance}
\input{./pbdMPI-include/04-faq}
\input{./pbdMPI-include/05-windows}
\input{./pbdMPI-include/06-analog}
